...
PERK.Hooks.PostEntityTakeDamage = function( ent, dmginfo, took )
    if not took or not ent:IsNPC() or not HORDE:IsBallisticDamage( dmginfo ) then return end

    local attacker = dmginfo:GetAttacker()
    if not attacker:IsPlayer() or not attacker:Horde_GetPerk( "juggernaut_base" ) then return end
...
PERK.Hooks.Horde_PlayerMoveBonus = function( ply, bonus_walk, bonus_run )
    ...
    local movementSlow = 0.4

    bonus_walk.increase = bonus_walk.increase + -movementSlow
    bonus_run.increase = bonus_run.increase + -movementSlow
end
