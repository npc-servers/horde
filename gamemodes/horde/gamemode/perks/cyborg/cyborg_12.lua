...
PERK.Hooks.Horde_OnPlayerDamagePost = function (ply, npc, bonus, hitgroup, dmginfo)
    if ply:Horde_GetPerk("cyborg_12") and HORDE:IsMeleeDamage(dmginfo) then
...
PERK.Hooks.Horde_OnPlayerDebuffApply = function (ply, debuff, bonus)
    if ply:Horde_GetPerk("cyborg_12") and debuff == HORDE.Status_Bleeding then
        bonus.apply = 0
        return true
    end
end
...
PERK.Hooks.Horde_OnEnemyKilled = function(victim, killer, inflictor)
	if not killer:Horde_GetPerk("cyborg_12") then return end
