...
PERK.Hooks.Horde_OnSetPerk = function(ply, perk)
    if SERVER and perk == "cyborg_11" then
...
PERK.Hooks.Horde_OnSetMaxHealth = function(ply, bonus)
    if SERVER and ply:Horde_GetPerk("cyborg_11") then
...
PERK.Hooks.Horde_OnPlayerDamageTaken = function (ply, dmginfo, bonus)
    if not ply:Horde_GetPerk("cyborg_11") then return end
...
PERK.Hooks.Horde_OnPlayerDebuffApply = function (ply, debuff, bonus)
    if ply:Horde_GetPerk("cyborg_11") and debuff == HORDE.Status_Break then
        bonus.apply = 0
        return true
    end
end