...
PERK.Hooks.PlayerTick = function( ply )
    ...
    if not ply.Horde_CyborgNinjaArmorLossCurtime then
        ply.Horde_CyborgNinjaArmorLossCurtime = 0
    end

    if plyInFrenzy and CurTime() >= ply.Horde_CyborgNinjaArmorLossCurtime then
        ply:SetArmor( math.max( 0, armor - armorLost ) )
        ply.Horde_CyborgNinjaArmorLossCurtime = CurTime() + armorLossRate
    end
