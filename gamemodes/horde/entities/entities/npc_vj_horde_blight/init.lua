...
function ENT:CustomOnTakeDamage_BeforeDamage(dmginfo, hitgroup)
    ...
    self:EmitSound("vj_acid/acid_splat.wav", 75, 100, 1)
    for _, ent in pairs(ents.FindInSphere(self:GetPos(), 200)) do
        if ent:IsPlayer() then
            local Trace = util.TraceLine({start = self:WorldSpaceCenter(), endpos = ent:WorldSpaceCenter(), mask = MASK_SOLID_BRUSHONLY})
